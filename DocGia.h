...
struct MuonTra
{
	...
	date ngayMuon;
	date ngayTra;
	int trangthai; //0-dang muon; 1-da tra; 2-da lam mat
};
...
struct docgia
{
	...
	string ten;
	string phai;
	int trangthaithe; // =0 khoa, =1 hoat dong, -1 chua hoat dong
	PTR_MT mt; // con tro se tro den danh sach cac cuon da va dang muon
	int nMT; //da muon bao nhieu sach
};
...
int ListMaThe[1024]={0,512,768,256,896,640,384,128,192,448,320,832,64,960,704,576,48,32,752,112,736,560,144,688,912,464,208,400,176,304,416,
...
{
	if(p== NULL) ThongBao("Khong tim thay");
	else
	{
		if(x < p->key) Xoa_DG(x, p->left);
		else if(x > p->key) Xoa_DG(x, p->right);
		else //x == p->key
		{
			rp=p;
			if(rp->right == NULL)  p= rp->left;
			else if(rp->left == NULL) p= rp->right;
			else Xoa_DG_3(rp->right);
			delete rp;
